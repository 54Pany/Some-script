...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	m_ServiceStatusHandle = RegisterServiceCtrlHandler("sbmproxy", 
		ServiceCtrlHandler); 
...
BOOL InstallService()
{
	...
	schService = CreateService(schSCManager,"sbmproxy", 
        "sbm proxy", // service name to display
	...
	SD.lpDescription = "代理服务器软件，全面支持http/socks4/socks5!,秒杀ccproxy，以快速著称。访问开发者博客：http://gf-zj.com 获得源代码、反馈问题"; 
...
BOOL DeleteService()
{
	...
	if (schSCManager == NULL)
		return false;
	hService=OpenService(schSCManager,"sbmproxy",SERVICE_ALL_ACCESS);
	if (hService == NULL)
		return false;
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		{{"sbmproxy",ServiceMain},{NULL,NULL}};
